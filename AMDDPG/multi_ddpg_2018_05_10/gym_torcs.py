...
class TorcsEnv:
    ...
    def __init__(self, vision=False, throttle=False, gear_change=False, render = False):
        self.vision = vision
    ...
    def step(self, u, track_name):
       #print("Step")
        ...
        #progress = sp * np.cos(obs['angle']) * ( 1 - np.abs(np.sin(obs['angle'])) ) * ( 1 -  np.abs(obs['trackPos']) )
        ...
        if client.S.laps == 0:
            ...
            dist_from_start = client.S.read_distFromStart()
        if client.S.laps == 1 and dist_from_start >= track_length_dict[track_name]-10:
            reward = 20000
        """    
	    ...
	    # to be commented during test
        if self.test == False and  self.terminal_judge_start < self.time_step: # Episode terminates if the progress of agent is small
        ...
        return self.get_obs(), reward, client.R.d['meta'], {}, episode_terminate
