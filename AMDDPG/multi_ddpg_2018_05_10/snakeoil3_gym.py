            ...
            try:
                ...
                if n_fail < 0:
                    ...
                    if self.vision is False:
                        os.system(u'torcs -nofuel -nodamage -nolaptime -T &')
                    else:
...
class ServerState(object):
    ...
    def __init__(self):
        ...
        self.d= dict()
	#[wjt]: for checking the laps
        self.laps = 0
        self.length_had_run = 0
        self.lastDistanceFromStart = 0.0
        self.lastDistanceFromStart_forCheckLaps = 0.0
        #self.preAngle = 0.0
        #self.preSteer = 0
    ...
    def check_laps(self):
        """
        this func have called it in self.getObservation()
        check this car are in which lap
        :return:  set laps into self.laps,
        """
        if self.d['distRaced'] < self.d['distFromStart']:
            self.laps = 0
        if self.lastDistanceFromStart_forCheckLaps > self.d['distFromStart']:  # and abs(self.d['angle']) < .1:
            self.laps += 1
        self.lastDistanceFromStart_forCheckLaps = self.d['distFromStart']

    #[wjt]
    def read_distFromStart(self):
        return self.d['distFromStart']
