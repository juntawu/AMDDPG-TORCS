...
file_path = "reward_5900_episode_Aalborg.json"
reward_file = open(file_path, 'r')
Reward = json.load(reward_file)
#num_steps = len(Reward[-1])
num_steps = max([len(reward) for reward in Reward])
...
plt.plot(Reward[0], '--', color='blue', label='sub-policy0', linewidth=2)
plt.plot(Reward[1], ':', color='green', label='sub-policy1', linewidth=2)
plt.plot(Reward[2], '-.', color='purple', label='sub-policy2', linewidth=2)
plt.plot(Reward[3], color='red', label='aggregated-policy', linewidth=2)
...
ltext = leg.get_texts()
plt.subplots_adjust(top = 0.98, bottom = 0.10, right = 0.90, left = 0.13, hspace = 0.20, wspace = 0.20)
plt.setp(ltext, fontsize=16)  #设置图例字体的大小和粗细
